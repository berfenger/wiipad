...
# #define[\s]+([A-Z0-9_]+)[\s]+([0-9a-zA-Z_\(\)+]+) ==> \1 = \2
...
def gettimeofday():
	...
	tt = timeval()
	tt.tv_sec = int(t)
	tt.tv_usec = int((t-int(t))*1000000)
	return tt
...
class ff_envelope(ctypes.Structure):
    _fields_ = [
        ...
        ("attack_level", ctypes.c_int16),
        ("fade_length", ctypes.c_int16),
        ("fade_level", ctypes.c_int16)
    ]
...
class ff_effect_union(ctypes.Union):
	_fields_ = [
        ...
        ("condition1", ff_condition_effect),
        ("condition2", ff_condition_effect),
...
# Get driver version
EVIOCGVERSION = ioctlhelp.IOR(ord('E'), 0x01, '@i')

# Get device ID
EVIOCGID = ioctlhelp.IOR(ord('E'), 0x02, ctypes.sizeof(input_id))

# Get repeat settings
EVIOCGREP = ioctlhelp.IOR(ord('E'), 0x03, '@ii')
# Set repeat settings
EVIOCSREP = ioctlhelp.IOW(ord('E'), 0x03, '@ii')

# Get keycode
EVIOCGKEYCODE = ioctlhelp.IOR(ord('E'), 0x04, '@ii')
# EVIOCGKEYCODE_V2 _IOR('E', 0x04, struct input_keymap_entry)

# Set keycode

EVIOCSKEYCODE = ioctlhelp.IOW(ord('E'), 0x04, '@ii')
#EVIOCSKEYCODE_V2 _IOW('E', 0x04, struct input_keymap_entry)

# Get device name
EVIOCGNAME = lambda _len: ioctlhelp.IOC(ioctlhelp._IOC_READ, ord('E'), 0x06,
        struct.calcsize('%ds' % _len))

# Get physical location
EVIOCGPHYS= lambda _len: ioctlhelp.IOC(ioctlhelp._IOC_READ, ord('E'), 0x07,
        struct.calcsize('%ds' % _len))

# Get unique identifier
EVIOCGUNIQ = lambda _len: ioctlhelp.IOC(ioctlhelp._IOC_READ, ord('E'), 0x08,
        struct.calcsize('%ds' % _len))

# Get device properties
EVIOCGPROP = lambda _len: ioctlhelp.IOC(ioctlhelp._IOC_READ, ord('E'), 0x09,
        struct.calcsize('%ds' % _len))

EVIOCGMTSLOTS = lambda _len: ioctlhelp.IOC(ioctlhelp._IOC_READ, ord('E'), 0x0a,
        struct.calcsize('%ds' % _len))
								
EVIOCGKEY = lambda _len: ioctlhelp.IOC(ioctlhelp._IOC_READ, ord('E'), 0x18,
        struct.calcsize('%ds' % _len))

EVIOCGLED = lambda _len: ioctlhelp.IOC(ioctlhelp._IOC_READ, ord('E'), 0x19,
        struct.calcsize('%ds' % _len))

EVIOCGSND = lambda _len: ioctlhelp.IOC(ioctlhelp._IOC_READ, ord('E'), 0x1a,
        struct.calcsize('%ds' % _len))

EVIOCGSW = lambda _len: ioctlhelp.IOC(ioctlhelp._IOC_READ, ord('E'), 0x1b,
        struct.calcsize('%ds' % _len))

EVIOCGBIT = lambda ev, _len: ioctlhelp.IOC(ioctlhelp._IOC_READ, ord('E'), 0x20 + ev, _len)

EVIOCGABS = lambda _abs: ioctlhelp.IOR(ord('E'), 0x40 + _abs, ctypes.sizeof(input_absinfo))

EVIOCSABS = lambda _abs: ioctlhelp.IOR(ord('E'), 0xc0 + _abs, ctypes.sizeof(input_absinfo))

# Force feedback
# Send a force effect to a force feedback device
EVIOCSFF = ioctlhelp.IOC(ioctlhelp._IOC_WRITE, ord('E'), 0x80, ctypes.sizeof(ff_effect))
# Erase a force effect
EVIOCRMFF = ioctlhelp.IOW(ord('E'), 0x81, '@i')
# Report number of effects playable at the same time
EVIOCGEFFECTS = ioctlhelp.IOR(ord('E'), 0x84, '@i')
# Grab/Release device
EVIOCGRAB = ioctlhelp.IOW(ord('E'), 0x90, 'i')

# Set clockid to be used for timestamps
EVIOCSCLOCKID = ioctlhelp.IOW(ord('E'), 0xa0, 'i')
...
"""print(ctypes.sizeof(ff_envelope))
print(ctypes.sizeof(ff_constant_effect))
print(ctypes.sizeof(ff_ramp_effect))
print(ctypes.sizeof(ff_periodic_effect))
print(ctypes.sizeof(ff_condition_effect))
print(ctypes.sizeof(ff_rumble_effect))
print(ctypes.sizeof(ff_replay))
print(ctypes.sizeof(ff_trigger))
print(ctypes.sizeof(ff_effect))"""
...
UI_DEV_CREATE = ioctlhelp.IO(UINPUT_IOCTL_BASE, 1)
UI_DEV_DESTROY = ioctlhelp.IO(UINPUT_IOCTL_BASE, 2)

#define UI_DEV_CREATE       _IO(UINPUT_IOCTL_BASE, 1)
#define UI_DEV_DESTROY      _IO(UINPUT_IOCTL_BASE, 2)

UI_SET_EVBIT = ioctlhelp.IOW(UINPUT_IOCTL_BASE, 100, '@i')
UI_SET_KEYBIT = ioctlhelp.IOW(UINPUT_IOCTL_BASE, 101, '@i')
UI_SET_RELBIT = ioctlhelp.IOW(UINPUT_IOCTL_BASE, 102, '@i')
UI_SET_ABSBIT = ioctlhelp.IOW(UINPUT_IOCTL_BASE, 103, '@i')
UI_SET_MSCBIT = ioctlhelp.IOW(UINPUT_IOCTL_BASE, 104, '@i')
UI_SET_LEDBIT = ioctlhelp.IOW(UINPUT_IOCTL_BASE, 105, '@i')
UI_SET_SNDBIT = ioctlhelp.IOW(UINPUT_IOCTL_BASE, 106, '@i')
UI_SET_FFBIT = ioctlhelp.IOW(UINPUT_IOCTL_BASE, 107, '@i')
UI_SET_PHYS = ioctlhelp.IOW(UINPUT_IOCTL_BASE, 108, '@i')
UI_SET_SWBIT = ioctlhelp.IOW(UINPUT_IOCTL_BASE, 109, '@i')
UI_SET_PROPBIT = ioctlhelp.IOW(UINPUT_IOCTL_BASE, 110, '@i')
...
UI_BEGIN_FF_UPLOAD = ioctlhelp.IOWR(UINPUT_IOCTL_BASE, 200, ctypes.sizeof(uinput_ff_upload))
UI_END_FF_UPLOAD = ioctlhelp.IOW(UINPUT_IOCTL_BASE, 201, ctypes.sizeof(uinput_ff_upload))
UI_BEGIN_FF_ERASE = ioctlhelp.IOWR(UINPUT_IOCTL_BASE, 202, ctypes.sizeof(uinput_ff_erase))
UI_END_FF_ERASE = ioctlhelp.IOW(UINPUT_IOCTL_BASE, 203, ctypes.sizeof(uinput_ff_erase))
...
print("UI_SET_EVBIT")
print(UI_SET_EVBIT)
print(UI_BEGIN_FF_UPLOAD)
print(UI_END_FF_UPLOAD)
print(UI_BEGIN_FF_ERASE)
print(UI_END_FF_ERASE)
print(ctypes.sizeof(uinput_ff_upload))
print(ctypes.sizeof(uinput_ff_erase))
print(ctypes.sizeof(uinput_user_dev))
t = gettimeofday()
print("sec  = "+repr(t.tv_sec))
print("usec = "+repr(t.tv_usec))
"""